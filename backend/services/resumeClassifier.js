function classifyResumeText(resumeText) {
  ...
  const hasTimeline =
    /\b(january|february|march|april|may|june|july|august|september|october|november|december|20\d{2}|19\d{2})\b/i.test(
      resumeText,
    );
  const hasSummary =
    /\b(summary|professional summary|about me|overview)\b/i.test(resumeText);
  const hasSkills =
    /\b(skills|technologies|technical skills|core competencies|tools)\b/i.test(
      resumeText,
    );
  const hasExperience =
    /\b(experience|employment|work history|professional history|projects|education)\b/i.test(
      resumeText,
    );
  const hasRoleSignal =
    /\b(software|developer|engineer|analyst|manager|intern|consultant|designer|architect)\b/i.test(
      resumeText,
    );
